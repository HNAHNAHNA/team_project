...
const UserFavorites = ({ userId }: { userId: number }) => {
    ...
    useEffect(() => {
        const fetchFavorites = async () => {
            setLoading(true);
            try {
                const res = await fetch(`/api/fastapi/favorites/${userId}`);
    ...
    const deleteFavoriteButtonClickHandler = async (userId: number, selectedData: Favorite) => {
        try {
            const res = await fetch(`/api/fastapi/favorites/delete?user_id=${userId}&accommodation_id=${selectedData.accommodation.accommodation_id}`,
                { method: "DELETE" }
            );
