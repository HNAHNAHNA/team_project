...
function HotelMap() {
  ...
  useEffect(() => {
    ...
    const fetchAllAccommodations = async () => {
      try {
        setLoading(true);
        const response = await fetch("http://15.164.129.209/api/fastapi/accommodations");
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      ...
      try {
        const res = await fetch(`http://15.164.129.209/api/fastapi/favorites/user/${user.id}`);
    ...
    try {
      if (isFav) {
        const res = await fetch(
          `http://15.164.129.209/api/fastapi/favorites/delete?user_id=${user.id}&accommodation_id=${hotelId}`,
          { method: 'DELETE' }
        );
        if (!res.ok) throw new Error("찜 해제 실패");
        setFavoriteMap((prev) => ({ ...prev, [hotelId]: false }));
      } else {
        const res = await fetch("http://15.164.129.209/api/fastapi/favorites", {
          method: "POST",
